...
export function label_from_location(
	...
	let resolved_header = resolve_header_label(address, header, data.parsed_file_bundle, data.notes_dir)
	if (resolved_header === undefined){
		console.warn("could not resolve header at ", address, ": ", header, " keeping the header label as-is")
...
export function resolve_header_label(address:string, header:string|string[], file_cache: note_cache, vault:Vault):string|undefined{
	...
	if(cached_content === undefined){
		...
		if(file === undefined || file_cache[file.basename] === undefined){
			if(file !== undefined && file_cache[file.basename] === undefined){
				console.warn("address of reference '", address, "' is referenced but was not embedded.")
			}
			const header_string = typeof header === "string"? header: header.join(".")
			console.warn("keeping the header address of ", address, ": ", header_string, " as-is")
	...
	if(typeof header === "string"){ // to make typescript happy
	...
	if (new_label === undefined){
		const header_string = typeof header === "string"? header: header.join(".")
		console.warn("Could not resolve header name '", header_string ,"' in file with address '", address, "', keeping the header label as-is")
		return header_string
	}
	return new_label
}
