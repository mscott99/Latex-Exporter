...
We \emph{have} \textbf{things} ``to" talk about. We can use latex macros: $\aff(T)$. We can reference markdown headers, which become references to latex sections: feel free to skip ahead to \autoref{loc:body.main_results}.

Results are referenced by linking notes: we will show that \autoref{loc:theorem_1.statement} holds by using \autoref{loc:lemma_1.statement}, \autoref{loc:lemma_2.statement}, \autoref{loc:other_small_lemmas.first_other_lemma} and \autoref{loc:other_small_lemmas.second_other_lemma}. We can state results through labelled embeds, as follows. 
\begin{lemma}
...
\begin{lemma}
\label{lem:explicit}
We reference the equation: \autoref{eq:main} and the result: \autoref{lem:explicit}.
\begin{align}
1+1 & = 1+3-2 \label{eq:aligned_eq:1}\\
& = 2 \label{eq:aligned_eq:2}
\end{align}
We can reference individual lines of an align environment: \autoref{eq:aligned_eq:1} and \autoref{eq:aligned_eq:2}.
\end{lemma}
...
The theorem environment is then referenced as \autoref{loc:theorem_1.statement}, or as \autoref{loc:theorem_1.statement}. To show \autoref{loc:theorem_1.statement}, we need the following two results.
\begin{lemma}
...
\begin{enumerate}
\item The first is \autoref{loc:other_small_lemmas.first_other_lemma}.
...
\begin{proof}[\hypertarget{loc:theorem_1.proof}Proof of \autoref{loc:theorem_1.statement}]
For this we require another result.
\begin{proposition}
\label{loc:lemma_2.statement}
$1+4 = 5$
\end{proposition}
\begin{proof}[\hypertarget{loc:lemma_2.proof}Proof of \autoref{loc:lemma_2.statement}]
Left to the reader.
\end{proof}
...
\end{figure}
We can reference the figures with a wikilink \autoref{fig:intro_comp_wlabel.pdf}.
\printbibliography
\appendix
\section{Appendix}
\begin{proof}[\hypertarget{loc:lemma_1.proof}Proof of \autoref{loc:lemma_1.statement}]
Ask chatgpt.
\end{proof}
\end{document}
